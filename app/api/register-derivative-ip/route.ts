...
export async function POST(request: NextRequest) {
  ...
  const { castId, castName, imagePath, walletAddress, prompt, originIpId } =
    await request.json();
  ...
  try {
    ...
    return new NextResponse(
      JSON.stringify({ message: "Registering derivative ip successful" }),
      {
        status: 200,
      }
    );
    ...
    // Send a success response
    return new NextResponse(
      JSON.stringify({ error: "Registering derivative ip successful" }),
      {
        status: 400,
      }
    );
